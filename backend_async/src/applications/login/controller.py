...
class LoginService:
    ...
    async def login(self, response: Response, username: str, password: str):
        ...
        user = await self.get_user(username_normalizado)
        if not user:
            raise HTTPException(
        ...
        await self.session_manager.create_session(username_normalizado, access_token)
    ...
    async def get_user(self, username: str) -> Optional[Users]:
        try:
            stmt  = select(Users).where(Users.usu == username)
    ...
    async def create_user(self, username: str, password: str, role: str):
        try:
            ...
            existing_user = await self.get_user(username_normalizado)
            if existing_user:
                return JSONResponse(
